...
def getPath(dataset_name):
    video_dir = "full/"
    path = "/gpfsdswork/projects/rech/urk/uln35en/raw_data/"+dataset_name+"/video"
    #path = "/storage/raid1/homedirs/alice.delbosc/data/"+dataset_name+"_data/raw_data/Video/"
    processed_dir = "processed/"

    return path, processed_dir, video_dir
...
def correctTimeAndTransitions(out, timestep, min_confidence):
    ...
    for csv_file in listdir(out):
        if(".csv" in csv_file):
            ...
            # Round the timestep to the closer multiple to 0.04
            df_video["timestamp"] = round(round(df_video["timestamp"] / timestep) * timestep,2) 
            # Group by timestep (some are duplicated) and then create the missing timestep by calculating the transitions  
...
if __name__ == "__main__":
    ...
    timestep = 0.04
    min_confidence = 0.6
    path, processed_dir, video_dir = getPath(dataset_name)

    dir = join(path, video_dir)
